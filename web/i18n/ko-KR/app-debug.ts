const translation = {
  ...
  feature: {
    ...
    dataSet: {
      title: '컨텍스트',
      noData: 'RAG를 컨텍스트로 가져올 수 있습니다',
      words: '단어',
      textBlocks: '텍스트 블록',
      selectTitle: '참조할 RAG 선택',
      selected: '선택한 RAG',
      noDataSet: 'RAG가 없습니다',
      toCreate: '생성하기',
      ...
      queryVariable: {
        ...
        unableToQueryDataSet: 'RAG를 쿼리할 수 없음',
        unableToQueryDataSetTip: 'RAG 쿼리에 실패했습니다. 정상적으로 쿼리할 수 없는 경우, 컨텍스트 섹션에서 컨텍스트 쿼리 변수를 다시 선택하십시오.',
        ok: '확인',
        contextVarNotEmpty: '컨텍스트 쿼리 변수를 비울 수 없습니다',
        deleteContextVarTitle: '변수 "{{varName}}"를 삭제하시겠습니까?',
        deleteContextVarTip: '이 변수는 컨텍스트 쿼리 변수로 설정되어 있어 삭제하면 RAG의 정상적인 사용에 영향을 미칩니다. 삭제하려면 컨텍스트 섹션에서 다시 선택하십시오.',
      },
    },
    tools: {
      title: '도구',
  ...
  datasetConfig: {
    ...
    knowledgeTip: 'RAG를 추가하려면 "+" 버튼을 클릭하세요.',
    retrieveOneWay: {
      title: 'N-to-1 리트리벌',
      description: '사용자 의도와 RAG 설명을 기반으로, 에이전트가 자율적으로 최적의 RAG를 선택합니다. 개별적이고 제한된 RAG를 가진 애플리케이션에 적합합니다.',
    },
    retrieveMultiWay: {
      title: '멀티패스 리트리벌',
      description: '사용자 의도에 따라 모든 RAG를 쿼리하고, 관련 텍스트를 여러 소스에서 가져와 다시 순위를 매긴 후 사용자 쿼리에 가장 적합한 결과를 선택합니다. 재순위 모델 API의 구성이 필요합니다.',
    },
    ...
    retrieveChangeTip: '인덱스 모드 및 리트리벌 모드를 변경하면 이 RAG와 관련된 애플리케이션에 영향을 줄 수 있습니다.',
  },
